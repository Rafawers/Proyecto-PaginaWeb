...
<head>
    ...
    <link rel="stylesheet" href="../css/styleCancelarDonacion.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.2/font/bootstrap-icons.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Pacifico&display=swap" rel="stylesheet">
    <title>Cancelar Donación</title>
</head>
    ...
    <div class="container">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($_SESSION['usuario'] == $correoUsuarioDonacion) {
                $query = "DELETE FROM donaciones WHERE id = $idDonacion";
                $resultado = mysqli_query($conexion, $query);

                if ($resultado) {
                    echo '
                        <script>
                            alert("Donación cancelada exitosamente");
                            window.location = "Donaciones.php";
                        </script>
                    ';
                } else {
                    echo "<p>Error al cancelar la donación. Inténtalo de nuevo.</p>";
                }
            } else {
                echo "<p>No tienes permiso para cancelar esta donación.</p>";
            }
            ...
            if ($filaDonacion = mysqli_fetch_assoc($consultaDonacion)) {
                ...
                echo "<p>Monto: {$filaDonacion['monto']}</p>";
                echo '<form action="' . $_SERVER['PHP_SELF'] . '" method="post">';
                echo '<input type="hidden" name="id_donacion" value="' . $filaDonacion['id'] . '">';
                echo '<input type="submit" value="Cancelar Donación">';
                echo '</form>';
            } else {
